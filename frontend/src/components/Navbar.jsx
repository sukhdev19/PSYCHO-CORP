...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-[#ADADAD] bg-white dark:bg-gray-900 dark:border-gray-700 dark:text-white'>
      <img onClick={() => navigate('/')} className='w-44 ml-5 cursor-pointer' src={assets.logo} alt="Logo" />
      <ul className='md:flex items-start gap-5 font-medium hidden ml-auto'> {/* Aligning items to the right */}
        <NavLink 
          to='/' 
          className={({ isActive }) => 
            ...
            <div className='flex items-center gap-2 cursor-pointer group relative'>
              ...
              <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block'>
                <div className='min-w-48 bg-gray-50 dark:bg-gray-800 dark:text-white rounded flex flex-col gap-4 p-4'>
                  <p onClick={() => navigate('/my-profile')} className='hover:text-black dark:hover:text-cyan-400 cursor-pointer'>My Profile</p>
                  <p onClick={() => navigate('/my-appointments')} className='hover:text-black dark:hover:text-cyan-400 cursor-pointer'>My Appointments</p>
                  <p onClick={logout} className='hover:text-black dark:hover:text-cyan-400 cursor-pointer'>Logout</p>
                </div>
              </div>
            </div>
          ) : (
